...
import { Plus, CreditCard as Edit2, Trash2, ArrowRight, GraduationCap, Users } from 'lucide-react';
    ...
    <Layout>
      <div className="mb-8">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-stone-800 mb-2">{levelName}</h1>
      ...
      {students.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-lg border-2 border-dashed border-stone-300">
          ...
          <h3 className="text-xl font-medium text-stone-800 mb-2">لا يوجد طلاب حتى الآن</h3>
          <p className="text-stone-600 mb-6">ابدأ بإضافة أول طالب في هذا المستوى</p>
          <button
            onClick={openCreateModal}
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-stone-800 text-lg leading-tight">{student.full_name}</h3>
                      <p className="text-sm text-stone-500 mt-1">{student.level_name}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="bg-stone-50 p-2 rounded-lg border border-stone-100">
                    ...
                    <p className="font-medium text-stone-800">{student.grade}</p>
                  </div>
                  <div className="flex items-center justify-center p-2">
                    <span className={`w-full text-center px-2 py-1.5 rounded-lg text-xs font-bold ${getResultColor(student.result)}`}>
                      {student.result}
                    </span>
                  </div>
                </div>
            ...
            <table className="w-full">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="text-right px-6 py-4 text-sm font-medium text-stone-700">المستوى</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-stone-700">الدرجة الكلية</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-stone-700">التقدير</th>
                  <th className="text-right px-6 py-4 text-sm font-medium text-stone-700">النتيجة</th>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-stone-800">{student.full_name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-stone-600">{student.level_name}</td>
                    <td className="px-6 py-4">
                      <span className="font-bold text-stone-800">{student.total}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-medium text-stone-700">{student.grade}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getResultColor(student.result)}`}>
                        {student.result}
                      </span>
                    </td>
                    <td className="px-6 py-4">
      ...
      {showDetailModal && selectedStudent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-stone-800 mb-6">تفاصيل الطالب</h2>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-stone-800">{selectedStudent.full_name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-stone-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-stone-800">{selectedStudent.activity}</p>
                </div>

                <div className="bg-stone-50 p-4 rounded-lg">
                  <p className="text-sm text-stone-600 mb-1">الشفوي</p>
                  <p className="text-2xl font-bold text-stone-800">{selectedStudent.oral}</p>
                </div>

                <div className="bg-stone-50 p-4 rounded-lg">
                  <p className="text-sm text-stone-600 mb-1">التحريري</p>
                  <p className="text-2xl font-bold text-stone-800">{selectedStudent.written}</p>
                </div>

                <div className="bg-amber-50 p-4 rounded-lg border-2 border-amber-200">
                  <p className="text-sm text-amber-700 mb-1">المجموع الكلي</p>
                  <p className="text-2xl font-bold text-amber-800">{selectedStudent.total}</p>
                </div>
              </div>

              <div className="bg-stone-50 p-4 rounded-lg">
                <p className="text-sm text-stone-600 mb-1">التقدير</p>
                <p className="text-xl font-bold text-stone-800">{selectedStudent.grade}</p>
              </div>

              <div className="bg-stone-50 p-4 rounded-lg">
                <p className="text-sm text-stone-600 mb-1">النتيجة</p>
                <span className={`inline-flex items-center px-4 py-2 rounded-lg text-lg font-bold ${getResultColor(selectedStudent.result)}`}>
